...
class Administrator:
    ...
    def __repr__(self):
        return "<Administrator('%s','%s','%s','%s')>" % (self.id, self.name, self.email, self.password)
    
    # Metodo para representar el objeto en una tupla
    def __str__(self):
        return "Administrator [id=%s, name=%s, email=%s, password=%s]" % (self.id, self.name, self.email, self.password)
